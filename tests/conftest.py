...
def transcript():
    ...
    for start in range(0, 50, 5):
        segments.append({
            "id": sid, "start": float(start), "end": float(start + 5),
            "text": f"Sentence number {sid} about our product.", "speaker": None,
            "words": [],
        })
        sid += 1
    for start in range(70, 120, 5):
        segments.append({
            "id": sid, "start": float(start), "end": float(start + 5),
            "text": f"Closing point {sid} for the audience.", "speaker": None,
            "words": [],
        })
        sid += 1
...
def brand():
    return {
        ...
        "logo": {"light": "/x/logo_white.png", "dark": "/x/logo.png",
                 "corner": "top-right", "margin_px": 48, "width_frac": 0.22},
        "subtitles": {"font_size": 64, "text_color": "#FFFFFF", "outline_color": "#000000",
                      "outline_px": 3, "highlight_color": "#3BA7F0"},
